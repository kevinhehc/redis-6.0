...
1）从节点在同步时，会清空所有数据，服务器在与主节点进行初连接时，数据库中的所有数据都将丢失，替换成主节点发送的数据。

2）Redis不支持主主复制。

3）主从复制不会阻塞master（不会阻塞master处理客户端请求），因为RDB是异步进行的；相反slave在初次同步数据时会阻塞不能处理客户端读请求。

4）当多个从节点尝试连接同一个主节点的时候，就会出现以下两种情况：

    4.1：如果master节点保存快照还未执行，所有从节点都会接收到相同的快照文件和相同缓冲区写命令。

    4.2：master节点保存快照正已经执行完毕，当主节点与较早的从节点完成以上全部步骤之后，
        主节点会跟新连接的从节点重新执行RDB保存快照。

5）如果从节点连接的时机不凑巧的话，进行多次RDB保存快照，会大量消耗主节点资源（CPU、内存和磁盘I/O资源）。
```